...
# 1. Business purpose
우리는 매일 반복되는 일상 속에서 느꼈던 감정이나 사건들을 기록하기 위해 일기를 씁니다. 하지만 텍스트 위주의 단순한 일기 작성은 지속적인 흥미를 유발하기 어렵고 결국 작심삼일에 그치는 경우가 많습니다. 또한 정서적 교감을 위해 반려동물을 키우고 싶어 하는 현대인은 많으나 주거 환경의 제약이나 경제적 부담 등으로 인해 실제 입양은 결코 쉬운 일이 아닙니다.

이러한 불편함과 한계를 해결하기 위해 일기 작성이라는 기록 행위를 반려동물 육성과 결합한 시스템 ‘LOG-PET’을 고안했습니다. 사용자가 오늘 있었던 일을 동물에게 들려주듯 일기를 작성하면 시스템은 이를 로그로 기록함과 동시에 동물의 스탯에 반영하여 함께 성장하게 합니다. 이를 통해서 사용자들은 직접 반려동물을 키우지 않아도 정서적 유대감을 경험할 수 있을 것이며 단순한 기록을 넘어 동물이 나의 일상을 기억하고 반응해 주는 특별한 교감 다이어리를 소유할 수 있을 것입니다.
...
10)일기 작성 및 교감
ACTOR-Owner, Storage DB
DESCRIPTION-(핵심) 사용자가 일상을 기록하면 텍스트 양에 따라 친밀도와 경험치가 상승하며 상호작용이 발생합니다.
...
Purpose: 사용자의 일상을 기록함과 동시에 반려동물과의 정서적 유대감 형성.

Approach: 사용자가 텍스트로 일기를 입력하면 시스템이 글자 수를 분석하여 친밀도(Affinity)와 경험치(EXP) 가중치를 부여하고 이를 로그로 저장합니다.

Dynamics: 사용자가 일상을 공유하고 싶을 때 발생하며, 작성 완료 시 동물의 즉각적인 반응(애니메이션 및 메시지)이 뒤따릅니다.

Goals: 일기 쓰기의 습관화 유도 및 디지털 생명체와의 실시간 상호작용 구현.
...
Storage DB: 회원 정보 및 동물의 성장 데이터, 일기 로를 파일 시스템 형태로 저장하고 관리하는 내부 모듈이다.
...
Activity Log : 사용자가 입력한 일기 텍스트와 작성 시간, 그때의 동물 상태를 구조화하여 저장한 데이터 단위이다. 추억 회상 기능의 데이터가 된다.
